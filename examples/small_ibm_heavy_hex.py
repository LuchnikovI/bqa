...
config = {
    ...
    "max_bp_iters_number" : 100,    # this is the maximal BP iterations allowed, if BP reaches this value, warning is shown and BP is terminated, it is optional, default value is 100
    "measurement_threshold" : 0.95, # this is a confedence thresold which allows to skip measurement simulation and directly project state to the corresponding direction, it is optional, default value is 0.95
    "seed" : 42,         # this is random seed, it is optional, default value is 42
    ...
    "schedule" : {                   # this field specifies the annealing schedule, it is optional, to see default value see the corresponding warning
        ...
        "starting_mixing" : 1.,      # starting mixing, i.e. simulation starts from starting_mixing * H_mix + (1 - starting_mixing) * H_int hamiltonian, it is optional, default value is 1.
        "actions" : [                # this is a list with actions (QA instructions), it is optional, to see the default value, see the corresponding warning message

            # the following instruction corresponds to annealing dynamics:
            {"time" : 1., # the part of the "physical" time dedicated to this instruction
             ...
             "final_mixing" : 0.0,  # the mixing after the instruction is finished, i.e. final hamiltonian is final_mixing * H_mix + (1 - final_mixing) * H_int hamiltonian
             },
            # instruction requesting to collect density matrices into a list
            "get_density_matrices",
            # instruction requesting to perform measurements of all nodes, results are collected as integer values into a list
            "measure",
        ]
    },
}
