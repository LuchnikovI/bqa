...
from .utils import (ConfigSyntaxError, analyse_0_to_1_number, analyse_non_neg_number,
                    analyse_positive_int, filtermap, ok_or_default_and_warn, is_sequential,
                    unwrap_or)
...
def analyse_linear_scaling(linear_scaling) -> LinearScaling:
    ...
    steps_number = ok_or_default_and_warn(
    ...
    return {"time" : analyse_0_to_1_number(time),
            "steps_number" : analyse_positive_int(steps_number),
            "final_mixing" : analyse_0_to_1_number(final_mixing)}
